...
import { Card, Image, Icon,  Dropdown, CardDescription } from 'semantic-ui-react'
  ...
  return (
    <div css={styles} onClick={onClickHandler}>
      <Card onClick={onClickHandler} >
        ...
        <Card.Content>
          <Card.Header>{props.title} <Icon name="users" css={iconStyle1}/><DropdownSettings/></Card.Header>
          <Card.Description>{"Owner: " + props.owner}</Card.Description>
          <Card.Description>{"Time Accessed: " + props.timeAccessed}</Card.Description>
          <Card.Description>{props.isShared ? <Icon name="users" /> : null }</Card.Description>
        </Card.Content>
      </Card>
    </div>
